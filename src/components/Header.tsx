...
const Link = withStyles<LinkProps>(styles)(
  ({ className, action, children, classes }) => (
    <a
      className={classNames([classes.headerLink, className])}
      href="#"
      onClick={(e) => {
        e.preventDefault();
        action();
 ...
 */
const Menu = withStyles<MenuProps>(styles)(({ label, children, classes }) => (
  <div className={classNames([classes.headerDropdown, "dropdown show"])}>
    <a
      className={classNames([classes.headerLink, "dropdown-toggle"])}
      href="#"
      target="_blank"
 ...
 */
const MenuItem = withStyles<MenuItemProps>(styles)(({ url, children }) => (
...
class Header extends React.PureComponent<
  StyledComponentProps<HeaderProps, typeof styles>,
  HeaderState
> {
  timer: any;
    ...
    return (
      <React.Fragment>
        <Menu label={user.profile.username}>
          ...
          <MenuItem url={QUICK_START_GUIDE_URL}>Quick Start Guide</MenuItem>
          <MenuItem url={formUrl}>Help / Feedback</MenuItem>
          <div className="dropdown-divider"></div>
  ...
  componentWillReceiveProps(nextProps: HeaderProps) {
    if (
      nextProps.saveCount > this.props.saveCount &&
  ...
  renderSaveNotification() {
    ...
    const lastSucceeded = this.props.lastRequestSucceeded.caseOf({
      just: (v) => v,
      nothing: () => false,
    });
  ...
  renderPackageTitle() {
    ...
    return (
      <span>
        ...
        <span className={classes.version}>v{course.version}</span>
      </span>
    );
  }
  ...
  render() {
    ...
    const label =
      this.isCourseRoute() && this.props.course
        ? this.renderPackageTitle()
        : this.renderApplicationLabel();
      ...
      <div className={classNames(["Header", classes.Header])}>
        <div className={classes.headerLogo}>
          <Link
            ...
            action={viewActions.viewAllCourses}
          >
        ...
        <ReactCSSTransitionGroup
          transitionName="saving"
          transitionEnterTimeout={250}
          transitionLeaveTimeout={500}
        >
          {this.renderSaveNotification()}
        </ReactCSSTransitionGroup>
        <div className={classes.headerUserProfile}>{this.renderAbout()}</div>
      </div>
    );
  }
}
