...
class OrganizationEditor extends AbstractEditor<models.CourseModel, OrganizationEditorProps, OrganizationEditorState> {
  ...
  createLinkerDialog() {
    if (this.state.los != null) {
      return (<LearningObjectiveLinker closeModal={this.closeModal.bind(this)}
                                       sourceData={this.toFlat(this.state.los, new Array<Linkable>())}
                                       modalIsOpen={this.state.modalIsOpen} target={this.state.orgTarget}/>);
    } else {
