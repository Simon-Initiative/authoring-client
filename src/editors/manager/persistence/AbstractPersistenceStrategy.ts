...
import {onFailureCallback, onSaveCompletedCallback, PersistenceStrategy} from "./PersistenceStrategy";
...
export abstract class AbstractPersistenceStrategy implements PersistenceStrategy {
            ...
            persistence.persistDocument(lockedDocument)
                ...
                .catch(err => {
                    if (remainingRetries === 0) {
                        ...
                        persistence.retrieveDocument(doc._courseId, doc._id)
                            .then(doc => {
    ...
    releaseLock(when: number) {
        ...
        if (this.writeLockedDocumentId !== null) {
            return persistence.retrieveDocument(this.courseId, this.writeLockedDocumentId)
                ...
                return new Promise((resolve, reject) => {
                    ...
                    this.setWriteLock(doc, new Date().getTime(), userId)
                        .then(lockedDocument => {
                            ...
                            this.courseId = lockedDocument._courseId;
